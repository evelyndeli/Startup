...
public class Startup {
    ...
    public static void main(String[] args) throws ParseException, IOException {
        ...
        String nom, correo, direccion, poblacion, provincia;
        Usuario usuario;
        ...
        //caso 5
        int idOjectoComprovar;
        
        // variables temporales caso 7
        String auxIdCambio, auxCosteCambio;    
        boolean costeCambioCorrecto, existeElObjetoCambiar = false;
        
        // Variables caso 8
        FileWriter fich;
        
        // variables caso 9
        String idPropietario9;
        boolean solonumeros9;
        int idobjeto9;
        
        //variables caso 10
        float suma = 0;
        Usuario usuarioaux;
        ArrayList<Usuario> sortedUsers = new ArrayList();
        
        do{
           opcion = ponerMenu(); // comprobamos que la entrada es un numero
            ...
            switch (opcion){
                case 1:
                    ...
                    if (correo != null) {
                        // Comprobamos que el correo tien la forma adecuada
                    ...
                    usuario = new Usuario(nom, correo, direccion, poblacion, provincia);
                ...
                case 2:
                    ...
                    while((propietarioExiste == false)){
                            if((auxIdPropietario < usuarios.size())){
                                if(!usuarios.get(auxIdPropietario).isBaja())
                ...
                case 3:
                    ...
                    for(int i = 0; i < auxObjetos.size(); i++){
                        ...
                        if (auxObjetos.get(i).getIdobjeto() == idObj && auxObjetos.get(i).isDisponible() && !usuarios.get(auxObjetos.get(i).getPropietario()).isBaja()){
                            idPropietarioObjeto = auxObjetos.get(i).getPropietario();
                            auxCoste = auxObjetos.get(i).getCoste();
                            auxNombre = usuarios.get(idPer - 1).getNombre(); // aqui tambien un -1 no?
                            ...
                            // lo añadimos al vector de prestamos
                            alquilarObjeto = new AlquilarObjeto(idPer, idObj, auxCoste,fecha_iniTemp, fecha_finTemp, auxNombre);
                            prestamos.add(alquilarObjeto);
                        }
                        else {
                ...
                case 6: 
                    ...
                    for(int i = 0; i < usuarios.size(); i++){
                        if(!usuarios.get(i).isBaja() && usuarios.get(i).tieneObjetos() && auxObjetos.get(i).getTieneAlquiler()){
                            System.out.println(usuarios.get(i).ToString());
                            usuarios.get(i).mostrarObjetos();
                ...
                case 7: 
                    System.out.println("RELLENE LOS DATOS PARA MODIFICAR EL COSTE DEL OBJETO \n"
                                        + "\tID del objeto:");
                    auxIdCambio = sc.nextLine();
                    
                    //comprobar que el objeto existe
                    while(!existeElObjetoCambiar){
                        for(int i = 0; i < auxObjetos.size(); i++){
                            if(Integer.valueOf(auxIdCambio) == auxObjetos.get(i).getIdobjeto()){
                                existeElObjetoCambiar = auxObjetos.get(i).isDisponible();
                            }
                        }
                        
                        if(!existeElObjetoCambiar){
                            System.out.println("RELLENE LOS DATOS PARA MODIFICAR EL COSTE DEL OBJETO \n"
                                        + "\tID del objeto:");
                            auxIdCambio = sc.nextLine();
                        }
                    ...
                    System.out.println("\tCoste por dia del objeto:");
                    auxCosteCambio = sc.nextLine();
                    costeCorrecto = soloNumeros(auxCosteCambio);
                    // comprobar que solo son numeros
                    while (costeCorrecto == false){
                        System.out.println("El coste del objeto tiene que ser un numero");
                        auxCosteDia = sc.nextLine();

                        costeCorrecto = soloNumeros(auxCosteDia);
                    }
                    auxcosteDia = Float.valueOf(auxCosteCambio); //pasamos el valor a float
                    ...
                    // aqui se añade el nuevo coste al objeto
                    auxObjetos.get(Integer.valueOf(auxIdCambio) - 1).modificarCoste(Float.valueOf(auxCosteCambio));
                ...
                case 10:
                    ...
                    for(int i = 0; i < prestamos.size(); i++){
                        ...
                        usuarioaux = usuarios.get(prestamos.get(i).getIdPersona()-1);
                        sortedUsers.add(usuarioaux);
                        //suma = suma + prestamos.get(i).calcularCoste();
                        
                    }
                    sortedUsers.stream()
                    .sorted(Comparator.comparing(Usuario::getSumCoste))
                    .collect(Collectors.toList());
                    
                    for(int j = 0; j < sortedUsers.size(); j++){
                        sortedUsers.get(j).ToString();
                    }
                ...
                default:System.out.println("Tiene que ser un número del 1 al 11"); 
    ...
    public static void mostrarMenu(){
        ...
        System.out.println("MENU \n"
                            ...
                            + "\t 5- Baja de objeto \n"
                            + "\t 6- Mostrar saldos \n"
                            + "\t 7- Modificar precio de un objeto \n"
                            + "\t 8- Guardar saldos en fichero  \n"
                            + "\t 9- Dar de baja un usuario \n"
                            + "\t 10- Mostrar mas freceuntes \n"
                            + "\t 11- Salir \n"
